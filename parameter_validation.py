    ...
    def areas(post):
        ...
        if not validation.post.has_key("areas"):
            ...
            validation.warnings.append(
                "Parameter 'areas' was missing, replaced with All")
            return validation
        # if area is empty, replace with all
        if validation.post["areas"] == []:
            validation.post["areas"] = ['All']
            validation.warnings.append(
                "Parameter 'areas' was empty list, added All")
            return validation
        # if area has wrong format it try to convert
        if type(validation.post["areas"]) != list:
            if type(validation.post["areas"]) in [str, unicode]:
                ...
                validation.warnings.append(
                    "Parameter area has the wrong format, converted from string to list")
            else:
                validation.warnings.append(
                    "[ERROR] Parameter area has the wrong format")
                return validation
        # if area not valid it ignores the area
        i = 0
        while i < len(validation.post["areas"]):
            ...
            if a.capitalize() not in AREAS:
                ...
                # correct uppercase and lowercase letters
                validation.post["areas"][i] = a.capitalize()
                i += 1
        # if dublicates in area, remove the latest
        if len(validation.post["areas"]) > len(set(validation.post["areas"])):
            validation.warnings.append(
                "dublicates in areas, remove the latest each")
            ...
            if post.get(key):
                if type(post.get(key)) == list and len(post[key]) >= 3:
                    try:
                        ...
                        # if 0..255
                        elif all(map(lambda c: 0 <= int(c) <= 255, post[key])):
                            post[key] = [float(c)/255 for c in post[key]]
                    except:
                        warnings.append(
                            "can not decode color parameter %s (tried as array)" % str(post[key]))
                        del post[key]
                elif type(post.get(key)) in [str, unicode]:
                    post[key] = str(post[key])
                    if post[key].find('#') == 0:
                        try:
                            post[key] = map(lambda x: int(
                                x, 16)/255.0, [post[key][1:3], post[key][3:5], post[key][5:7]]) + [1]
                        except:
                            warnings.append(
                                "can not decode color parameter %s (tried as hex)" % str(post[key]))
                            del post[key]
                        ...
                        for data in COLORS:  # find in color names
                            if data['name'].lower() == post[key].strip().lower():
                                post[key] = map(
                                    lambda x: x/255.0, list(eval(data['rgb']))) + [1]
                                success = True
                                break
                        if not success:
                            warnings.append(
                                "Cannot find the color name %s" % post[key])
                            del post[key]

                else:
                    warnings.append(
                        "can not decode color parameter %s" % str(post[key]))
                    del post[key]
        ...
        if post.get("fadespeed"):
            try:
                ...
                if f <= 1 and post.get("interval"):
                    # TODO make sure that interval is validated
                    f = f * float(post["interval"])
                    warnings.append(
                        "interpreted fadespeed as a relative value, converted to absolute value depending on interval")
                post["fadespeed"] = f
